...
fn end_to_end_storage_workflow() -> anyhow::Result<()> {
    ...
    assert_eq!(hp2.read_tuple(slot3).unwrap(), b"product:laptop:price:999.99");
...
fn multiple_files_multiple_pages() -> anyhow::Result<()> {
    ...
    let read_hp3 = HeapPage { page: dm.read_page(file3_page1)? };
    let read_hp1_1 = HeapPage { page: dm.read_page(file1_page1)? };
    let read_hp2 = HeapPage { page: dm.read_page(file2_page1)? };
    let read_hp1_2 = HeapPage { page: dm.read_page(file1_page2)? };
...
fn large_dataset_handling() -> anyhow::Result<()> {
    ...
    println!("Successfully stored and verified {} pages with ~{} tuples each", 
             pages.len(), all_slots[0].len());
